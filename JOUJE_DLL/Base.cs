...
namespace JOUJE_DLL
{
    ...
    public class Base
    {
        ...
        const string connexionStringAzure = @"Server=tcp:srv-jm-jouje.database.windows.net,1433;Initial Catalog=JoujeV2;Persist Security Info=False;User ID=joujeadmin;Password=Jouje30.;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;";
        public Base()
        {
            using(SqlConnection connection = new SqlConnection(connexionStringAzure))
            {
                connection.Open();
            }
        }
        ...
        {
            using (var connection = new SqlConnection(connexionStringAzure))
            {
                connection.Open();
        ...
        public void UpdateGuitare(ref Guitare _guitare)
        {
            using (SqlConnection connection = new SqlConnection(connexionStringAzure))
            {
                ...
                connection.Query<Guitare>("UpdateGuitare", parameters, commandType: CommandType.StoredProcedure);
            }
        }

        public void UpdateGuitare(int idGuitare, string nomGuitare, int idClient, int idMicro, int idMicro1, int idMicro2, int idBois, int idBois1, int idBois2, int idVibrato)
        {
            using (SqlConnection connection = new SqlConnection(connexionStringAzure))
            {
                connection.Open();
                ...
                var parameters = new DynamicParameters();
                parameters.Add("@guitareID", idGuitare);
                parameters.Add("@nomGuitare", nomGuitare, DbType.String, ParameterDirection.Input);
                parameters.Add("@clientID", idClient, DbType.Int16, ParameterDirection.Input);
                parameters.Add("@micro_1_ID", idMicro, DbType.Int16, ParameterDirection.Input);
                parameters.Add("@micro_2_ID", idMicro1, DbType.Int16, ParameterDirection.Input);
                parameters.Add("@micro_3_ID", idMicro2, DbType.Int16, ParameterDirection.Input);
                parameters.Add("@bois_1_ID", idBois, DbType.Int16, ParameterDirection.Input);
                parameters.Add("@bois_2_ID", idBois1, DbType.Int16, ParameterDirection.Input);
                parameters.Add("@bois_3_ID", idBois2, DbType.Int16, ParameterDirection.Input);
                parameters.Add("@vibratoID", idVibrato, DbType.Int16, ParameterDirection.Input);
